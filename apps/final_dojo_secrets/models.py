...
class SecretManager(models.Manager):
    def validate(self, postedSecret, userid):
        # postedSecret = postData['secret']
        if len(postedSecret) < 4:
            return(False, "Secrets must be longer than 4 characters, bruh! You know you done fucked up!")
        try:
            ...
            return(False, "We could not create your secret at this time")

    def newlike(self, secretid, userid):
        #Determine that the right secret is being liked
        try:
    ...
    def deletelike(self, secretid, userid):
        #Determine that the correct secret is being deleted
        try:
            secret = self.get(id=secretid)
        except:
            return(False, "This secret is not found in our database, bruh!")
        user = User.objects.get(id=secretid)
        if secret.author != user:
            return(False, "Shame on you, bruh! You can't (and shouldn't) delete your own secrets!")
        secret.delete()
